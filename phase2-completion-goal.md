...
Use this order whenever instructions, todos, or API ideas conflict:

1. Wire compatibility is a non-negotiable floor. Generated bytes and decoder
   behavior must remain compatible with official SBE for the same logical
   message.
2. Performance comes first among valid designs. ErgoSBE must be comparable to or
   faster than Aeron SBE on the measured hot paths. Do not accept an ergonomic
   API, safety wrapper, abstraction, validation pass, allocation, branch, or
   trait layer that makes the hot path slower without an explicit opt-in.
3. Rust ergonomics and safety come next, but only when they are zero-cost or kept
   off the hot path. Prefer Rust-native affordances such as typestate, sealed
   traits, associated types, HRTBs, GATs where stable and justified,
   `TryFrom`, proof tokens, and transparent newtypes when they make the API
   easier or safer without regressing performance.
4. Simplicity wins over cleverness when performance and safety are equal.
...
- Generated code is not checked in except the golden stability target.
- Codegen uses `syn`, `quote!`, and `prettyplease`; do not add
  `push_str(&format!(...))`.
- Decoders are always version-aware; tail offsets use the wire block length.
- Do not transmute from buffers.
- Unsafe fast paths are opt-in through explicit policy or feature flags.
- Done means verified by a running test or repeatable command.
...
- Real completion requires a repeatable head-to-head Aeron comparison command or
  recipe. If it is missing, add it rather than deferring.
