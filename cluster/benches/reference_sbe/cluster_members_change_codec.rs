...
pub use decoder::ClusterMembersChangeDecoder;
pub use encoder::ClusterMembersChangeEncoder;
...
pub mod encoder {
    ...
    pub struct ClusterMembersChangeEncoder<'a> {
        buf: WriteBuf<'a>,
    ...
    impl<'a> Writer<'a> for ClusterMembersChangeEncoder<'a> {
        #[inline]
    ...
    impl<'a> Encoder<'a> for ClusterMembersChangeEncoder<'a> {
        #[inline]
    ...
    impl<'a> ClusterMembersChangeEncoder<'a> {
        ...
        /// VAR_DATA ENCODER - character encoding: 'US-ASCII'
        #[inline]
        pub fn passive_members(&mut self, value: &[u8]) -> &mut Self {
            let limit = self.get_limit();
            let data_length = value.len().min((u32::MAX - 1) as usize);
            self.set_limit(limit + 4 + data_length);
            self.get_buf_mut().put_u32_at(limit, data_length as u32);
...
pub mod decoder {
    ...
    pub struct ClusterMembersChangeDecoder<'a> {
        buf: ReadBuf<'a>,
        initial_offset: usize,
        offset: usize,
        limit: usize,
    ...
    impl ActingVersion for ClusterMembersChangeDecoder<'_> {
        #[inline]
    ...
    impl<'a> Reader<'a> for ClusterMembersChangeDecoder<'a> {
        #[inline]
    ...
    impl<'a> Decoder<'a> for ClusterMembersChangeDecoder<'a> {
        #[inline]
        fn get_limit(&self) -> usize {
            self.limit
        }
    ...
    impl<'a> ClusterMembersChangeDecoder<'a> {
        ...
        /// VAR_DATA DECODER - character encoding: 'US-ASCII'
        #[inline]
        pub fn passive_members_decoder(&mut self) -> (usize, usize) {
            let offset = self.get_limit();
            let data_length = self.get_buf().get_u32_at(offset) as usize;
            self.set_limit(offset + 4 + data_length);
            (offset + 4, data_length)
        ...
        #[inline]
        pub fn passive_members_slice(&'a self, coordinates: (usize, usize)) -> &'a [u8] {
            debug_assert!(self.get_limit() >= coordinates.0 + coordinates.1);
            self.get_buf().get_slice_at(coordinates.0, coordinates.1)
        }
    }
