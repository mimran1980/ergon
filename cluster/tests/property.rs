...
proptest! {
    ...
    fn prop_challenge_roundtrip(
        ...
        let dec = ergo_aeron_cluster::codecs::session::ChallengeDecoder::try_wrap_and_apply_header(bytes, 0).unwrap();
    ...
    fn prop_new_leader_roundtrip(
        ...
        let dec = ergo_aeron_cluster::codecs::session::NewLeaderEventDecoder::try_wrap_and_apply_header(bytes, 0).unwrap();
        prop_assert_eq!(dec.leadership_term_id(), ltid);
        prop_assert_eq!(dec.cluster_session_id(), csid);
    ...
    fn prop_session_event_roundtrip(
        ...
        let dec = ergo_aeron_cluster::codecs::session::SessionEventDecoder::try_wrap_and_apply_header(bytes, 0).unwrap();
        prop_assert_eq!(dec.cluster_session_id(), csid);
        prop_assert_eq!(dec.correlation_id(), cid);
        prop_assert_eq!(dec.leadership_term_id(), ltid);
        prop_assert_eq!(dec.leader_member_id(), mid);
