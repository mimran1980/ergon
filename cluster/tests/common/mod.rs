...
///
/// Egress URI is built via [`ergo_aeron_cluster::uri::udp_endpoint_cstr`] so it
...
pub fn connect_own_driver(
    ...
    let egress = ergo_aeron_cluster::uri::udp_endpoint_cstr(&format!("localhost:{egress_port}"))?;
