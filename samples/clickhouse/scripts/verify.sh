...
for uid in $uids; do
    ...
    table=$(echo "$dash" | jq -r '.templating.list[]? | select(.name=="table") | .current.value // empty')
    ...
    while IFS= read -r target; do
        ...
        raw=${raw//'${table:singlequote}'/"'${table:-trade}'"}
        raw=${raw//'${table}'/${table:-trade}}
        raw=${raw//'${sql:raw}'/$adhoc}
        body=$(jq -n --arg sql "$raw" --argjson fmt "$(echo "$target" | jq '.format')" \
        ...
        [[ -z $err ]] || fail "Grafana panel '$title' ($uid): $err"
        rows=$(echo "$result" | jq '[.results.A.frames[]?.data.values[0]? // [] | length] | add // 0')
        # book_snapshot starts disabled, so its panels may legitimately be empty.
        [[ $rows -gt 0 || $title == *book_snapshot* ]] || fail "Grafana panel '$title' ($uid) returned no rows"
        checked=$((checked + 1))
...
ok "all $checked Grafana panels query without error and return rows"
