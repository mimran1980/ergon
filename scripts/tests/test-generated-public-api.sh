...
tmp=$(mktemp)
snap="$root/api/generated/car_lean.txt"
cp "$snap" "$tmp"
trap 'mv "$tmp" "$snap"' EXIT
grep -v 'serial_number' "$tmp" > "$snap"
if cargo test -p ergo-sbe --test generated_public_api_test manifest_fixtures_match_snapshots --offline -- --exact >/dev/null 2>&1; then
