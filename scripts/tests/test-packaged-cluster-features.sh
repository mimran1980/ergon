...
if [[ "$output" != *"advertised explode feature is not buildable"* && "$output" != *"error"* && "$output" != *"FAIL"* ]]; then
    echo "wrong failure; expected a build/check error, got:" >&2
    echo "$output" >&2
    exit 1
fi

echo "test-packaged-cluster-features: PASS (fail-closed unpack)"
