#!/usr/bin/env bash
# run-cluster-instruction-probes.sh — mechanism-level evidence for cluster
# codec hot paths.
#
# Sibling to run-sbe-instruction-probes.sh (see that file's header for the
# full rationale) applied to `ergo-aeron-cluster`: Criterion measures
# wall-clock on one machine at one moment. It cannot show whether a call was
# emitted or how many instructions each arm retired. This lane answers those
# questions for cluster pairs that carry a documented wall-clock noise-floor
# allowance in scripts/check-bench-gate.sh (see NOISE_FLOOR_CEILING_EXCEPTIONS
# in sbe/benchmarks/tests/bench_gate_test.rs).
#
# It drives the `cluster_perf_probe` binary (cluster/benches/cluster_perf_probe.rs)
# under system Valgrind with raw Callgrind, collecting ONLY the named probe
# symbol so setup and validation — which run before the probe is entered — are
# excluded. It then disassembles the exact same binary with the pinned
# llvm-objdump.
#
# A separate script rather than a shared one, deliberately: cluster codecs
# live in a different package with a different fixture story
# (cluster/benches/reference_sbe/ is a plain module, not a separate crate),
# matching this repo's existing split between run-sbe-bench.sh and the
# bench-cluster wall-clock recipe.
#
# Usage:
#   run-cluster-instruction-probes.sh --profile lto
#   run-cluster-instruction-probes.sh --profile no-lto
#   run-cluster-instruction-probes.sh --all-profiles --topic decode
#   run-cluster-instruction-probes.sh --profile lto --probe ergo_probe_decode_session_event
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MANIFEST="$REPO_ROOT/cluster/probes.tsv"
PROFILES=()
TOPIC=""
PROBES=()
...
RUN_ID="cp$(date -u +%Y%m%dT%H%M%SZ)-$$"
OUT_ROOT="$REPO_ROOT/target/cluster-instruction-probes/$RUN_ID"
mkdir -p "$OUT_ROOT"
...
echo "=== cluster instruction probes: run $RUN_ID ==="
echo "commit=$COMMIT"
...
for profile in "${PROFILES[@]}"; do
    ...
    echo ""
    echo "=== building cluster_perf_probe — $profile ==="
    if [ "$profile" = "no-lto" ]; then
        env CARGO_TARGET_DIR="$target_dir" \
            CARGO_PROFILE_RELEASE_LTO=false \
            CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1 \
            cargo build --release -p ergo-aeron-cluster --bin cluster_perf_probe
    else
        env CARGO_TARGET_DIR="$target_dir" \
            cargo build --release -p ergo-aeron-cluster --bin cluster_perf_probe
    fi
    binary="$target_dir/release/cluster_perf_probe"

    # `rusteron-client`'s build script places libaeron next to the build, not
    # on the default loader search path. `cargo build`/`cargo run` inject this
    # automatically; a binary launched directly (as Valgrind and the --list
    # check below both do) does not get it for free. Read the same
    # cargo:rustc-link-search=native=… line cargo itself reads from the build
    # script's own log, so this tracks whatever rusteron-client emits rather
    # than a guessed or hardcoded path.
    build_log=$(find "$target_dir/release/build" -maxdepth 1 -name 'rusteron-client-*' -type d | head -1)
    [ -n "$build_log" ] || {
        echo "FAIL: no rusteron-client build output under $target_dir/release/build" >&2
        exit 1
    }
    lib_dir=$(grep '^cargo:rustc-link-search=native=' "$build_log/output" | head -1 | sed 's/^cargo:rustc-link-search=native=//')
    [ -n "$lib_dir" ] || {
        echo "FAIL: rusteron-client build output named no link-search directory" >&2
        exit 1
    }
    ...
    if ! diff -u "$MANIFEST" "$profile_dir/probes-actual.tsv" > "$profile_dir/manifest.diff"; then
        echo "FAIL: cluster_perf_probe registry does not match $MANIFEST" >&2
    ...
    llvm-objdump --disassemble --demangle "$binary" > "$profile_dir/cluster-perf-probe.objdump"
    ...
    for probe in "${selected[@]}"; do
        ...
        if ! grep -q "<$probe>" "$profile_dir/cluster-perf-probe.objdump"; then
