...
fn criterion_benchmark(c: &mut criterion::Criterion) {
    let now_ns: i64 = black_box(1_720_000_000_000_000_000); // ~2024-07
    let now_dt = chrono::DateTime::from_timestamp_nanos(now_ns);

    // Wire → DateTime (decode path)
    c.bench_function("chrono/nanos_to_datetime", |b| {
        b.iter(|| {
            let dt = ergo_sbe::chrono_converters::i64_nanos_to_datetime(black_box(now_ns));
            black_box(dt)
        });
    });
    ...
    let now_us: i64 = black_box(1_720_000_000_000_000); // ~2024-07 in micros
    c.bench_function("chrono/micros_to_naive", |b| {
        b.iter(|| {
            let dt = ergo_sbe::chrono_converters::i64_micros_to_naive(black_box(now_us));
            black_box(dt)
        });
    });

    // NaiveDateTime → wire (encode path, micros)
    let now_naive = ergo_sbe::chrono_converters::i64_micros_to_naive(now_us);
