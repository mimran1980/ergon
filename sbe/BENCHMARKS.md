...
`just bench` writes under `target/bench-runs/<run-id>/`, not a table in prose.
Quote a result by naming its run id, commit, host, rustc, profile, and
manifest hash — or do not quote it.
