...
pub use decoder::TestMessage2Decoder;
pub use encoder::TestMessage2Encoder;
...
pub mod encoder {
    ...
    pub struct TestMessage2Encoder<'a> {
        buf: WriteBuf<'a>,
    ...
    impl<'a> Writer<'a> for TestMessage2Encoder<'a> {
        #[inline]
    ...
    impl<'a> Encoder<'a> for TestMessage2Encoder<'a> {
        #[inline]
    ...
    impl<'a, P> EntriesEncoder<P> where P: Encoder<'a> + Default {
        ...
        #[inline]
        pub fn var_data_field_1(&mut self, value: &str) -> &mut Self {
        ...
        /// VAR_DATA ENCODER - character encoding: 'UTF-8'
        #[inline]
        pub fn var_data_field_2(&mut self, value: &str) -> &mut Self {
            let limit = self.get_limit();
            let data_length = value.len().min((u8::MAX - 1) as usize);
            self.set_limit(limit + 1 + data_length);
            self.get_buf_mut().put_u8_at(limit, data_length as u8);
...
pub mod decoder {
    ...
    pub struct TestMessage2Decoder<'a> {
        buf: ReadBuf<'a>,
        initial_offset: usize,
        offset: usize,
        limit: usize,
    ...
    impl ActingVersion for TestMessage2Decoder<'_> {
        #[inline]
    ...
    impl<'a> Reader<'a> for TestMessage2Decoder<'a> {
        #[inline]
    ...
    impl<'a> Decoder<'a> for TestMessage2Decoder<'a> {
        #[inline]
        fn get_limit(&self) -> usize {
            self.limit
        }
    ...
    impl<'a, P> EntriesDecoder<P> where P: Decoder<'a> + ActingVersion + Default {
        ...
        #[inline]
        pub fn var_data_field_1_decoder(&mut self) -> (usize, usize) {
        ...
        #[inline]
        pub fn var_data_field_1_slice(&'a self, coordinates: (usize, usize)) -> &'a [u8] {
        ...
        /// VAR_DATA DECODER - character encoding: 'UTF-8'
        #[inline]
        pub fn var_data_field_2_decoder(&mut self) -> (usize, usize) {
            let offset = self.parent.as_ref().expect("parent missing").get_limit();
            let data_length = self.get_buf().get_u8_at(offset) as usize;
            self.parent.as_mut().unwrap().set_limit(offset + 1 + data_length);
            (offset + 1, data_length)
        ...
        #[inline]
        pub fn var_data_field_2_slice(&'a self, coordinates: (usize, usize)) -> &'a [u8] {
            debug_assert!(self.get_limit() >= coordinates.0 + coordinates.1);
            self.get_buf().get_slice_at(coordinates.0, coordinates.1)
        }
