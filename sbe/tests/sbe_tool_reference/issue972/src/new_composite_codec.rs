...
pub mod encoder {
    ...
    impl<'a, P> NewCompositeEncoder<P> where P: Writer<'a> + Default {
        ...
        #[inline]
        pub fn f1_opt(&mut self, value: Option<u16>) -> &mut Self {
            match value {
                Some(value) => self.f1(value),
                None => self.f1(0x0_u16),
            };
            self
        }
        ...
        /// Set to `None` to encode the field null value.
        #[inline]
        pub fn f2_opt(&mut self, value: Option<u32>) -> &mut Self {
            match value {
                Some(value) => self.f2(value),
                None => self.f2(0x0_u32),
            };
            self
        }
...
pub mod decoder {
    ...
    impl<'a, P> Reader<'a> for NewCompositeDecoder<P> where P: Reader<'a> + ActingVersion + Default {
        #[inline]
    ...
    impl<'a, P> NewCompositeDecoder<P> where P: Reader<'a> + ActingVersion + Default {
        pub fn wrap(mut self, parent: P, offset: usize) -> Self {
            self.parent = Some(parent);
            self.offset = offset;
            self
        ...
        /// primitive field - 'OPTIONAL' { null_value: '0x0_u32' }
        #[inline]
        pub fn f2(&self) -> Option<u32> {
            if self.acting_version() < 1 {
                return None;
            }
