...
pub mod encoder {
    ...
    impl<'a> Message1Encoder<'a> {
        ...
        /// GROUP ENCODER (id=73, description='Repeating group containing orders')
        #[inline]
        pub fn list_ord_grp_encoder(self, count: u8, list_ord_grp_encoder: ListOrdGrpEncoder<Self>) -> ListOrdGrpEncoder<Self> {
            list_ord_grp_encoder.wrap(self, count)
        }

    }

    #[derive(Debug, Default)]
    pub struct ListOrdGrpEncoder<P> {
        parent: Option<P>,
        count: u8,
        index: usize,
        offset: usize,
    ...
    impl<'a, P> Writer<'a> for ListOrdGrpEncoder<P> where P: Writer<'a> + Default {
        #[inline]
        fn get_buf_mut(&mut self) -> &mut WriteBuf<'a> {
    ...
    impl<'a, P> Encoder<'a> for ListOrdGrpEncoder<P> where P: Encoder<'a> + Default {
        #[inline]
        fn get_limit(&self) -> usize {
    ...
    impl<'a, P> ListOrdGrpEncoder<P> where P: Encoder<'a> + Default {
        #[inline]
        pub fn wrap(
            mut self,
            mut parent: P,
        ...
        /// - description: Must be unique, by customer, for the day
        /// - min value: 32
        /// - max value: 126
        /// - null value: 0_u8
        ...
        pub fn cl_ord_id_zero_padded(&mut self, value: &[u8]) -> &mut Self {
            let iter = value.iter().copied().chain(std::iter::repeat(0_u8)).take(14);
...
pub mod decoder {
    ...
    impl<'a> Message1Decoder<'a> {
        ...
        /// GROUP DECODER (id=73, description='Repeating group containing orders')
        #[inline]
        pub fn list_ord_grp_decoder(self) -> ListOrdGrpDecoder<Self> {
            ListOrdGrpDecoder::default().wrap(self)
        }

    }

    #[derive(Debug, Default)]
    pub struct ListOrdGrpDecoder<P> {
        parent: Option<P>,
        block_length: u16,
        count: u8,
        index: usize,
    ...
    impl<'a, P> ActingVersion for ListOrdGrpDecoder<P> where P: Reader<'a> + ActingVersion + Default {
        #[inline]
        fn acting_version(&self) -> u16 {
    ...
    impl<'a, P> Reader<'a> for ListOrdGrpDecoder<P> where P: Reader<'a> + Default {
        #[inline]
        fn get_buf(&self) -> &ReadBuf<'a> {
    ...
    impl<'a, P> Decoder<'a> for ListOrdGrpDecoder<P> where P: Decoder<'a> + ActingVersion + Default {
        #[inline]
        fn get_limit(&self) -> usize {
            self.parent.as_ref().expect("parent missing").get_limit()
        }
    ...
    impl<'a, P> ListOrdGrpDecoder<P> where P: Decoder<'a> + ActingVersion + Default {
        pub fn wrap(
            mut self,
            mut parent: P,
        ) -> Self {
        ...
        /// group token - Token{signal=BEGIN_GROUP, name='ListOrdGrp', referencedName='null', description='Repeating group containing orders', packageName='null', id=73, version=0, deprecated=0, encodedLength=14, offset=4, componentTokenCount=9, encoding=Encoding{presence=REQUIRED, primitiveType=null, byteOrder=LITTLE_ENDIAN, minValue=null, maxValue=null, nullValue=null, constValue=null, characterEncoding='null', epoch='null', timeUnit=null, semanticType='null'}}
        #[inline]
        pub fn parent(&mut self) -> SbeResult<P> {
            self.parent.take().ok_or(SbeErr::ParentNotSet)
        }
