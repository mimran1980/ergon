...
fn all_enums_as_option_flat_group_bulk_compiles_and_nulls() -> Result<(), Box<dyn std::error::Error>>
{
  ...
  <message name="M" id="1">
    ...
    let src_selector = Generator::new(
        GenerationConfig::new("reviewgsel")
            .with_domain_objects(DomainVarData::Bytes)
            .with_null_as_option(ergo_sbe::ConversionSelector::named_type("Status")),
    )
    .generate(&schema)?
    ...
    compile_and_run(
        ...
        let buffer = &mut storage[..len];
        none.encode(buffer)?;
        assert_eq!(MDomain::try_from_slice_with_header(buffer, 0)?, none);
        assert_eq!(
            MRowsEntryDomain { status: None }.to_wire_entry().status,
            Status::NullVal
        );
...
/// Nested-entry DTO `None` must write `NullVal`. The message and flat-group
/// cells do not prove this location.
#[test]
fn nested_entry_enum_none_writes_null() -> Result<(), Box<dyn std::error::Error>> {
    use ergo_sbe::{DomainVarData, GenerationConfig, Generator, Schema, parse};
    const XML: &str = r#"<messageSchema package="reviewn" id="1" version="0" byteOrder="littleEndian">
  <types>
    <composite name="messageHeader">
      <type name="blockLength" primitiveType="uint16"/>
      <type name="templateId" primitiveType="uint16"/>
  ...
  <message name="M" id="1">
    <group name="rows" id="1" dimensionType="groupSizeEncoding">
      <group name="cells" id="2" dimensionType="groupSizeEncoding">
        <field name="status" id="3" type="Status"/>
      </group>
    </group>
  </message>
</messageSchema>"#;
    ...
    compile_and_run(
        ...
        r#"
        let some = MDomain {
            rows: vec![MRowsEntryDomain {
                cells: vec![MRowsEntryCellsEntryDomain { status: Some(Status::Done) }],
            }],
        };
        let none = MDomain {
            rows: vec![MRowsEntryDomain {
                cells: vec![MRowsEntryCellsEntryDomain { status: None }],
            }],
        };
        let len = some.encoded_length_with_header()?;
...
/// A versioned required enum is `Option` on the DTO because an old wire may
/// omit it. Encoding `None` into a current-version reused buffer must still
/// write `NullVal`.
#[test]
fn versioned_required_enum_none_writes_null_on_reused_buffer()
-> Result<(), Box<dyn std::error::Error>> {
    use ergo_sbe::{DomainVarData, GenerationConfig, Generator, Schema, parse};
    const XML: &str = r#"<messageSchema package="reviewv" id="1" version="1" byteOrder="littleEndian">
  <types>
    <composite name="messageHeader">
      <type name="blockLength" primitiveType="uint16"/>
      <type name="templateId" primitiveType="uint16"/>
  ...
  <message name="M" id="1">
    <field name="status" id="1" type="Status" sinceVersion="1"/>
  </message>
</messageSchema>"#;
    let schema = Schema::from_ir(parse(XML)?);
    let src =
        Generator::new(GenerationConfig::new("reviewv").with_domain_objects(DomainVarData::Bytes))
            .generate(&schema)?
            .modules()
            .next()
            .expect("one module")
    ...
    compile_and_run(
        ...
        let mut buffer = [0u8; MEncoder::compute_length_with_header()];
        MDomain { status: Some(Status::Done) }.encode(&mut buffer)?;
        MDomain { status: None }.encode(&mut buffer)?;
        let actual = MDomain::try_from_slice_with_header(&buffer, 0)?;
        assert_eq!(
            actual.status,
            Some(Status::NullVal),
            "versioned required None must write NullVal; without null_as_option the getter keeps the variant"
        );
        "#,
    );
    Ok(())
