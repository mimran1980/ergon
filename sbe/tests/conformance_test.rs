...
fn conformance_fixed_only_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        r#"
        let mut buf = [0u8; 64];
        let mut enc = FixedOnlyEncoder::wrap_and_apply_header(&mut buf, 0).unwrap();
        enc.id(42).price(10000).qty(10).side(Side::Buy);
        let encoded = enc.as_ref();

        let dec = FixedOnlyDecoder::try_from(encoded).unwrap();
...
fn conformance_flat_group_known_known() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        .description(b"test exchange data").unwrap();

        let encoded = complete.as_bytes();

        let dec = FlatGroupDecoder::try_from(encoded).unwrap();
        ...
        let after_asks = asks.finish().unwrap();
        let (desc, _complete) = after_asks.into_description().unwrap();
...
fn conformance_flat_group_unknown_unknown() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let after_asks = asks.finish().unwrap();
        let (desc, _c) = after_asks.into_description().unwrap();
        assert_eq!(desc, b"uu");
...
fn conformance_length_builder_invariants() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let body_len = FlatGroupEncodedLength::new()
            .bids(2, |b| { b.add()?; b.add()?; Ok(()) }).unwrap()
            .asks(1, |a| { a.add()?; Ok(()) }).unwrap()
            .description(17).unwrap()
            .encoded_length();
...
fn conformance_nested_group_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut enc = NestedGroupEncoder::wrap_and_apply_header(&mut buf, 0).unwrap();
        ...
        .asks(1, |asks| {
            asks.add(|entry| {
                ...
                entry.venue(b"NYSE").unwrap();
                Ok(())
            })?;
            Ok(())
        }).unwrap()
        .comment(b"test nested group").unwrap();

        let encoded = complete.as_bytes();

        let dec = NestedGroupDecoder::try_from(encoded).unwrap();
        assert_eq!(dec.exchange_id(), 8888, "exchange_id");

        // NestedGroupBidsDecoder yields Result<NestedGroupBidsEntryDecoder, _>
        let mut bids = dec.into_bids().unwrap();
        let bid_entries: Vec<_> = bids.by_ref().collect();
        assert_eq!(bid_entries.len(), 1, "expected 1 bid");
        let b0 = bid_entries[0].as_ref().unwrap();
        assert_eq!(b0.price(), 5000, "bid.price");
        assert_eq!(b0.qty(), 100, "bid.qty");

        // Nested orders: NestedGroupBidsOrdersDecoder yields direct entries
        let mut b0_orders = b0.orders().unwrap();
        let b0_order_entries: Vec<_> = b0_orders.by_ref().collect();
        ...
        // Entry-level varData after iterating orders
        assert_eq!(b0.venue().unwrap(), b"NASDAQ", "bids.venue");

        // NestedGroupAsksDecoder yields Result<NestedGroupAsksEntryDecoder, _>
        let after_bids = bids.finish().unwrap();
        let mut asks = after_bids.into_asks().unwrap();
        let ask_entries: Vec<_> = asks.by_ref().collect();
        assert_eq!(ask_entries.len(), 1, "expected 1 ask");
        let a0 = ask_entries[0].as_ref().unwrap();
        assert_eq!(a0.price(), 5001, "ask.price");
        assert_eq!(a0.qty(), 50, "ask.qty");

        // Ask orders: direct entries
        ...
        assert_eq!(a0.venue().unwrap(), b"NYSE", "asks.venue");

        let after_asks = asks.finish().unwrap();
        let (comment, _complete) = after_asks.into_comment().unwrap();
...
fn conformance_all_types_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        .payload(b"binary payload data").unwrap();

        let encoded = complete.as_bytes();

        let dec = AllTypesDecoder::try_from(encoded).unwrap();
        ...
        // AllTypesEntriesDecoder yields direct entries
        let mut ents = dec.into_entries().unwrap();
        ...
        let after_ents = ents.finish().unwrap();
        let (payload, _complete) = after_ents.into_payload().unwrap();
...
fn conformance_pure_fixed_nested_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        r#"
        let mut buf = vec![0u8; 4096];
        let mut enc = PureFixedNestedEncoder::wrap_and_apply_header(&mut buf, 0).unwrap();
        enc.id(42u64);
        ...
        let dec = PureFixedNestedDecoder::try_from(encoded).unwrap();
        assert_eq!(dec.id(), 42, "id");

        // PureFixedNestedRecordsDecoder yields Result<...> items
        let mut records = dec.into_records().unwrap();
        let record_vec: Vec<_> = records.by_ref().collect();
        assert_eq!(record_vec.len(), 2, "expected 2 records");

        let r0 = record_vec[0].as_ref().unwrap();
        assert_eq!(r0.key(), 100, "record[0].key");
        assert_eq!(r0.value(), 10, "record[0].value");

        // PureFixedNestedRecordsTagsDecoder yields direct entries
        let r0_tags = r0.tags().unwrap();
...
fn conformance_var_data_edge_cases() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut enc1 = FlatGroupEncoder::wrap_and_apply_header(&mut buf1, 0).unwrap();
        enc1.symbol(1);
        let complete1 = enc1.bids(0, |_| Ok(())).unwrap()
            .asks(0, |_| Ok(())).unwrap()
            .description(b"").unwrap();
        let encoded1 = complete1.as_bytes();
        let dec1 = FlatGroupDecoder::try_from(encoded1).unwrap();
        let bids1 = dec1.into_bids().unwrap();
        let after_bids1 = bids1.finish().unwrap();
        let asks1 = after_bids1.into_asks().unwrap();
        let after_asks1 = asks1.finish().unwrap();
        let (desc, _c) = after_asks1.into_description().unwrap();
        assert_eq!(desc, b"", "empty description");

        // UTF-8 string via varStringEncoding
        let mut buf2 = vec![0u8; 256];
        let mut enc2 = FlatGroupEncoder::wrap_and_apply_header(&mut buf2, 0).unwrap();
        enc2.symbol(2);
        let complete2 = enc2.bids(0, |_| Ok(())).unwrap()
            .asks(0, |_| Ok(())).unwrap()
            .description("Hello, \u{4e16}\u{754c}!".as_bytes()).unwrap();
        let encoded2 = complete2.as_bytes();
        let dec2 = FlatGroupDecoder::try_from(encoded2).unwrap();
        let bids2 = dec2.into_bids().unwrap();
        let after_bids2 = bids2.finish().unwrap();
        let asks2 = after_bids2.into_asks().unwrap();
        let after_asks2 = asks2.finish().unwrap();
        let (desc2, _c2) = after_asks2.into_description().unwrap();
        assert_eq!(desc2, "Hello, \u{4e16}\u{754c}!".as_bytes(), "utf-8 description");

        // as_str roundtrip via varStringEncoding
        let dec3 = FlatGroupDecoder::try_from(encoded2).unwrap();
        let bids3 = dec3.into_bids().unwrap();
        let after_bids3 = bids3.finish().unwrap();
        let asks3 = after_bids3.into_asks().unwrap();
        let after_asks3 = asks3.finish().unwrap();
        let (desc_str, _c3) = after_asks3.into_description_as_str().unwrap();
...
fn conformance_fixed_raw_fixed_parity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        r#"
        let mut buf1 = [0u8; 64];
        let mut enc1 = FixedOnlyEncoder::wrap_and_apply_header(&mut buf1, 0).unwrap();
        enc1.id(42).price(10000).qty(10).side(Side::Buy);
        let bytes1 = enc1.as_ref();

        let mut buf2 = [0u8; 64];
        let mut enc2 = FixedOnlyEncoder::wrap_and_apply_header(&mut buf2, 0).unwrap();
        ...
        let d1 = FixedOnlyDecoder::try_from(bytes1).unwrap();
        let d2 = FixedOnlyDecoder::try_from(bytes2).unwrap();
