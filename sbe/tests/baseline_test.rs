...
fn bounds_checks_disabled_with_feature_flag() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_with_feature(
        ...
        &src,
        r#"
        // With bound-check-disabled, wrap_and_apply_header (unchecked).unwrap() uses unsafe fast path
        let mut buf = [0u8; 512];
        let mut car = CarEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        car.serial_number(1234);
        car.model_year(2013);
...
fn static_header_templates_exist() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let _ = CarEncoder::<'_>::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        assert_eq!(
            &buf[0..8],
...
fn encoder_wrap_short_buffer_returns_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let _encoder = CarEncoder::try_wrap_and_apply_header(&mut exact, 0).unwrap();
    "#,
    );
    Ok(())
}
...
fn incomplete_encoder_has_no_complete_bytes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_fails_with_diagnostics(
        ...
        r#"
        let mut buf = [0u8; 512];
        let mut encoder = CarEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
...
fn forward_compat_v2_decoder_reads_v1_bytes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        r#"
        let mut buf = [0u8; 256];
        let mut e = versmsg_v1::VersionedMessageV1Encoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        e.field_a1(100);
        e.field_b1(200);
...
fn backward_compat_v1_decoder_reads_v2_bytes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        r#"
        let mut buf = [0u8; 256];
        let mut e = versmsg_v2::VersionedMessageV2Encoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        e.field_a1(42);
        e.field_b1(99);
        e.field_c2(111);
        e.field_d2(222);
...
fn framecursor_iterates_length_prefixed_frames() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        r#"
        let mut buf = [0u8; 512];
        let mut car1 = CarEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        ...
        let mut car2 = CarEncoder::try_wrap_and_apply_header(&mut buf[e1.len()..], 0).unwrap();
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        r#"
        let mut buf = [0u8; 256];
        let mut e = grpvers_v1::GroupMsgEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        r#"
        let mut buf = [0u8; 256];
        let mut e = grpvers_v2b::GroupMsgEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        let after_entries = e.entries(2, |g| {
...
#[test]
fn generated_encoder_header_typestate_not_wire_stage_generic()
-> Result<(), Box<dyn std::error::Error>> {
    let (_schema, src) = generate(&Paths::example_schema(), MODULE);
    // Header mode uses a zero-sized marker (HeaderPresent/HeaderAbsent) so
    // as_bytes_with_header is only available when the header was applied.
    // Wire-order stages remain named monomorphic structs — not Encoder<State>.
    assert!(
        src.contains("core::marker::PhantomData"),
        "encoder stages must carry PhantomData<H> for HeaderState"
    );
    assert!(
        src.contains("HeaderState = sbe_rt::HeaderPresent"),
        "encoder stages default H = HeaderPresent (no turbofish on happy path)"
    );
    assert!(
        !src.contains("car_encoder_state"),
        "encoder must not use car_encoder_state module (no generic wire-stage state)"
    );
    // Wire-order stages stay named structs. Reject a generic `State = …`
    // parameter, but allow `HeaderState = HeaderPresent` (header mode only).
    assert!(
        !src.contains("State =") || src.contains("HeaderState ="),
        "encoder must not use a wire-order State generic (HeaderState default is OK)"
    );
    assert!(
        !src.contains("car_encoder_state::"),
        "no car_encoder_state module paths"
    );
    Ok(())
}
...
fn header_typestate_happy_path_and_body_only_ergonomics() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    compile_and_run(
        "header_ergonomics_ok",
        &src,
        r#"
        // Stack buffer without turbofish on H (default HeaderPresent).
        let mut buf = [0u8; 512];
        ...
        enc.vehicle_code([b'a', b'b', b'c', b'd', b'e', b'f']);
        enc.extras(OptionalExtras::default());
        enc.engine(Engine::new(
            2000,
            4,
            [b'1', b'2', b'3'],
            35,
        ...
        let enc = enc.fuel_figures(0, |_| Ok(())).unwrap();
        let enc = enc.performance_figures(0, |_| Ok(())).unwrap();
        let enc = enc.manufacturer(b"X").unwrap();
        let enc = enc.model(b"Y").unwrap();
        let complete = enc.activation_code(b"").unwrap();

        // Helper with defaulted H — no turbofish, no HeaderPresent in signature.
        fn finish(enc: CarComplete<'_>) -> usize {
            enc.encoded_length_with_header()
        }
        assert_eq!(CarSchema::TEMPLATE_ID, CarEncoder::TEMPLATE_ID);
        assert_eq!(CarSchema::SCHEMA_ID, CarEncoder::SCHEMA_ID);
        // HeaderPresent slice methods are available on the complete stage.
        let _ = complete.as_bytes_with_header();
        let _ = complete.as_body_bytes();
        let n = finish(complete);
        assert!(n >= CarEncoder::HEADER_LENGTH);
        "#,
    );

    // (d) — body-only wrap yields HeaderAbsent; as_bytes_with_header is absent.
    compile_fails_with_diagnostics(
        "header_ergonomics_body_only",
        &src,
        r#"
        let mut buf = [0u8; 512];
...
fn nested_message_decode_via_vardata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut inner = InnerEncoder::try_wrap_and_apply_header(&mut inner_buf, 0).unwrap();
        inner.value(42);
        ...
        let mut outer = OuterEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
        outer.trace_id(7);
...
fn manual_start_entry_matches_closure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut car_c = CarEncoder::try_wrap_and_apply_header(&mut buf_closure, 0).unwrap();
        ...
        let closure_bytes = car_c.activation_code(b"").unwrap().as_bytes_with_header().to_vec();

        let mut car_m = CarEncoder::try_wrap_and_apply_header(&mut buf_manual, 0).unwrap();
        ...
        let manual_bytes = car_m.activation_code(b"").unwrap().as_bytes_with_header().to_vec();
...
fn decimal_converter_composite_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        r#"
        let mut buf = [0u8; 256];
        let mut enc = OrderEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap();
...
fn decimal_converter_wire_and_generic_byte_identity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut enc_wire = OrderEncoder::try_wrap_and_apply_header(&mut buf_wire, 0).unwrap();
...
fn fixed_method_exists_and_is_functional() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !src.contains("fn finish(") || src.contains("finish(self)"),
        "encoder finish() must NOT be generated (no fixed-phase bypass); group decoder finish is fine"
    );
    assert!(
        src.contains("struct CarFixedFields"),
...
fn fixed_method_manual_equivalence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        &src,
        r#"
        let mut buf_direct = [0u8; 256];
        let mut buf_fixed = [0u8; 256];
        let mut d = CarEncoder::try_wrap_and_apply_header(&mut buf_direct, 0).unwrap();
        ...
        let direct = d.activation_code(b"X").unwrap().as_bytes_with_header().to_vec();
        ...
        let f = CarEncoder::try_wrap_and_apply_header(&mut buf_fixed, 0).unwrap();
        let f = f.fixed(&ff);
        ...
        let fixed = f.activation_code(b"X").unwrap().as_bytes_with_header().to_vec();
...
fn nested_message_rejects_malformed_payload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut encoder = InnerEncoder::try_wrap_and_apply_header(&mut tmp_inner, 0).unwrap();
        encoder.value(42);
        ...
        let mut bad_outer = OuterEncoder::try_wrap_and_apply_header(&mut bad_buf, 0).unwrap();
...
<sbe:message name="Book" id="1">
  ...
  <group name="levels" id="2" dimensionType="groupSizeEncoding">
    ...
    compile_and_run(
        ...
        // Raw wire model
        let mut buf_wire = [0u8; 256];
        let mut enc = BookEncoder::try_wrap_and_apply_header(&mut buf_wire, 0).unwrap();
        ...
        let mut enc = BookEncoder::try_wrap_and_apply_header(&mut buf_gen, 0).unwrap();
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let len1 = MsgEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap()
            .fixed(&MsgFixedFields { f: 42u32 })
            .encoded_length_with_header();
        let msg1 = AnyMessage::try_decode(&buf[..len1], 0)?;
        ...
        let len2 = MsgMessageEncoder::try_wrap_and_apply_header(&mut buf2, 0).unwrap()
            .fixed(&MsgMessageFixedFields { f: 99u32 })
            .encoded_length_with_header();
        let msg2 = AnyMessage::try_decode(&buf2[..len2], 0)?;
...
fn any_message_encode_rejects_short_buffer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let len = MsgEncoder::try_wrap_and_apply_header(&mut buf, 0).unwrap()
            ...
            .encoded_length_with_header();
        let msg = AnyMessage::try_decode(&buf[..len], 0)?;
...
fn auto_bool_domain_works_with_arbitrary_bool_enum_name() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    compile_and_run(
        ...
        // Wire type accessor is renamed to *_wire when conversion/domain is active:
        assert_eq!(dec.approved_wire(), YesNo::Yes);
        // Domain try_* and legacy _bool both expose bool:
        let approved: bool = dec.try_approved()?;
        assert!(approved);
        assert!(dec.approved_bool());
        "#,
    );
    Ok(())
}
