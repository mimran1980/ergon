...
fn decode_baseline_fixture() -> Result<(), Box<dyn std::error::Error>> {
    run_fixture_test(
        ...
        r#"
        let car = CarDecoder::decode(FIXTE, 0).unwrap();
...
fn static_header_templates_exist() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let _ = CarEncoder::<'_>::wrap_and_apply_header(&mut buf, 0).unwrap();
        assert_eq!(
            &buf[0..8],
...
fn encoder_wrap_short_buffer_returns_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        assert!(CarEncoder::wrap(&mut scratch, usize::MAX).is_err());
        assert!(CarEncoder::wrap_and_apply_header(&mut scratch, usize::MAX).is_err());
        assert!(CarDecoder::decode(&scratch, usize::MAX).is_err());
...
fn forward_compat_v2_decoder_reads_v1_bytes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        let d = versmsg_v2::VersionedMessageV2Decoder::decode(encoded, 0).unwrap();
...
fn backward_compat_v1_decoder_reads_v2_bytes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run_two_modules(
        ...
        let d = versmsg_v1::VersionedMessageV1Decoder::decode(encoded, 0).unwrap();
...
fn nested_message_decode_via_vardata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let outer_decoder = OuterDecoder::decode(&buf, 0).unwrap();
...
fn nested_message_as_message_requires_ordered_consumption() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    compile_fails_with_diagnostics(
        ...
        let dec = OuterDecoder::decode(&buf, 0).unwrap();
...
fn decimal_converter_composite_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let dec = OrderDecoder::decode(&encoded, 0).unwrap();
        let price = dec.price();
...
fn conversion_only_domain_dto_uses_wire_setters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let dec = OrderDecoder::decode(&wire, 0).unwrap();
...
fn decimal_converter_wire_and_generic_byte_identity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        // Verify wire decode
        let dec_wire = OrderDecoder::decode(&wire_bytes, 0).unwrap();
...
fn nested_message_rejects_malformed_payload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let bad_dec = OuterDecoder::decode(&bad_buf, 0).unwrap();
...
<sbe:message name="Book" id="1">
  ...
  <group name="levels" id="2" dimensionType="groupSizeEncoding">
    ...
    compile_and_run(
        ...
        let dec = BookDecoder::decode(&gen_bytes, 0).unwrap();
...
fn decimal_converter_exact_adapter_matrix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        for &(m, e, scaled) in cases {
            ...
            let dec = OrderDecoder::decode(&buf, 0).unwrap();
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let len1 = MsgEncoder::wrap_and_apply_header(&mut buf, 0).unwrap()
        ...
        let len2 = MsgMessageEncoder::wrap_and_apply_header(&mut buf2, 0).unwrap()
...
fn auto_bool_domain_works_with_arbitrary_bool_enum_name() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    compile_and_run(
        ...
        let len = VoteEncoder::wrap_and_apply_header(&mut buf, 0)?
