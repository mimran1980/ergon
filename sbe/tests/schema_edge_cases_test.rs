...
fn basic_group_bulk_add_encodes_and_checks_boundaries() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut buf = [0u8; 128];
        let len = TestMessage1Encoder::wrap_and_apply_header(&mut buf, 0).unwrap()
        ...
        let len = TestMessage1Encoder::wrap_and_apply_header(&mut full_buf, 0).unwrap()
            .entries(1, |group| {
        ...
        let err = TestMessage1Encoder::wrap_and_apply_header(&mut short_buf, 0).unwrap()
        ...
        let len = TestMessage1Encoder::wrap_and_apply_header(&mut empty_buf, 0).unwrap()
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut buf = [0u8; 12];
        let len = ZeroBlockMessageEncoder::wrap_and_apply_header(&mut buf, 0).unwrap()
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let len = VersionedGroupMessageEncoder::wrap_and_apply_header(&mut latest, 0).unwrap()
            .entries(1, |group| {
...
fn multi_nested_group_compiles_and_roundtrips() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_and_run(
        ...
        let mut enc = MultiNestedEncoder::wrap_and_apply_header(&mut buf, 0).unwrap();
        enc.header(0u32);
