...
- [x] ✅ `acting_version()` and `acting_block_length()` exposed
  ...
  - Constness is acceptable here because these are pure stored-header metadata
    reads, not generated byte-helper hot paths.
...
- [x] ⚠️ Primitive scalar accessors were previously `const fn`, but the current
      design no longer treats constness as a goal for runtime buffer reads.
      Prefer inline Aeron-style read helpers over const-only byte loops.
...
- ✅ Constant-value fields and metadata/layout helpers may be `const fn`
- ⚠️ Field read accessors that touch runtime buffers should not be forced to
  remain `const fn`; use fast runtime byte helpers instead
- ⚠️ `_unchecked` variants do not need to be `const unsafe fn` unless their
  implementation remains identical to the runtime fast path
- ✅ `acting_version()`, `acting_block_length()` can stay `const fn` when they
  read stored header fields without calling non-const byte helpers
...
**Strongest:** Wire layout, E3 enum pattern, inline runtime accessors,
safe-by-default, optional/null semantics.
