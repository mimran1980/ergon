...
- [x] `const fn` for all consts (allow inline in const contexts) — emitted as `pub const`, not `pub const fn`
- [x] Tests: verify consts match schema values

Ref: gap analysis (todo 51), DECISIONS.md §2.


## Verification / Unit Testing
- [x] Create unit tests `test_null_min_max_constants` verifying that `*_NULL`, `*_MIN`, and `*_MAX` constants are generated for every scalar field and match the SBE defaults or XML overrides.
