...
- [x] Composite field encode/decode matches upstream bytes (blocked by nested composite references, arrays, and inline types; see `100-nested-composites-and-refs.md`)
- [x] Closure sub-encoders for composites (`encode_foo(|e| { e.set_bar(...) })`)
...
- [x] Optional/null matrix: required/optional × since-version × null-sentinel
...
- [x] Create a unit test `test_composite_enum_set` that encodes and decodes composites, enums, and sets, verifying all discriminants, bitmasks, and field values match the expected SBE specification.
- [x] Add a unit test `test_closure_sub_encoders` that exercises composite writing via closures and verifies the output bytes.
 strategy
