...
- [x] Every field accessor that reads a fixed-size slice is `const fn`
- [x] `const fn BLOCK_LENGTH: usize` — already on SbeMessage
- [x] `const fn ENCODED_LENGTH: usize` — header + block length
- [x] `const fn entry_size() -> usize` on groups
...
- [x] `const fn` on every eligible method
