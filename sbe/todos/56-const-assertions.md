...
- [x] Create a compile-fail test verifying that changing the expected sizes or block lengths of composites in the schema causes a compile error via the generated const assertions (baseline test at line 510 is a string-pattern check, not a true compile-fail test).
