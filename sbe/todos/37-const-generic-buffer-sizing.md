...
- [x] `const ENCODED_LENGTH: usize` — exact for fixed messages
- [x] `const MAX_ENCODED_LENGTH: usize` — worst-case (header + block + max_groups * entry + max_var_data)
- [x] Fixed message: `ENCODED_LENGTH` == `MAX_ENCODED_LENGTH` (only `ENCODED_LENGTH` emitted per design — value is identical)
- [x] Variable message: `ENCODED_LENGTH` omitted (compile error by absence, not custom message per design)
- [x] Doc example on each message: `let mut buf = [0u8; Msg::MAX_ENCODED_LENGTH];`
- [x] `encoded_length(&self)` runtime method returns exact encoded size for the current message

## Verification / Unit Testing
- [x] Create a unit test `test_const_generic_buffer_sizing` in `sbe/tests/integration_tests.rs` or `smoke_test.rs` that:
