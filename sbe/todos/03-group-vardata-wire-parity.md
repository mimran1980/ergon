...
- [x] Var-data: `as_slice()`, `as_str()` → `Result<&str, Utf8Error>`, `as_decoder()`, `as_message()`
- [x] `unsafe fn as_str_unchecked()` for zero-cost UTF-8 skip
...
- [x] Create a unit test `test_repeating_groups_and_vardata` that encodes and decodes nested repeating groups and variable-length data, verifying tail offset calculations relative to acting version and wire blockLength.
- [x] Add a unit test `test_var_data_as_str` that decodes a var-data field as a string slice and validates correct UTF-8 mapping, and returns a DecodeError on invalid UTF-8.
 strategy
