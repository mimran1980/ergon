...
- [x] Nested `<composite>` definitions inside composites are parsed and emitted
- [x] Circular composite refs are rejected with a miette diagnostic pointing at
      the ref chain
- [x] Duplicate composite member names are rejected with labels on the original
      and duplicate member
- [x] Explicit composite offsets are validated against overlap/insufficient
