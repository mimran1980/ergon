...
**Status: ACTIVE / AERON QC RELEASE GATE**
...
- [x] Full diff of ErgoSBE `car_example.rs` vs Aeron `aeron_car.rs` completed
- [x] Every divergence catalogued with rationale ("copy Aeron" or "deliberate departure")
- [x] Byte-by-byte loops replaced with `get_bytes_at`-style helper (or ReadBuf)
- [x] Array reads use bulk `copy_from_slice` / `as_chunks` like Aeron
- [x] Bounds-check gating centralized (not duplicated per accessor)
- [x] Encoder closure borrow issue resolved (Aeron uses different pattern)
- [x] Generated code size comparable (within 20% of Aeron line count)
- [x] Perf benchmark: ErgoSBE within 10% of Aeron decode speed
- [x] Perf benchmark: no measured scenario where Aeron beats ErgoSBE; any gap has a blocking bug
- [x] Parser parity todos 125 and 126 are complete, or every remaining parser
      divergence from Aeron is documented as intentional with tests
- [x] **Re-verify all previously-done todos** after each major codegen change — a todo marked `[x]` may have been verified on stale generated code. The golden file is the ground truth; if the golden file changed, re-check every `[x]` item that depends on it.
