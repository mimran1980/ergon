...
fn generate_message_decoder(
    ...
    // Associated constants in concrete impl — resolves without turbofish
    ts.extend(quote::quote! {
        impl #decoder_ident<'_> {
...
fn generate_message_encoder(
    ...
    let wrap_apply_body = quote::quote! {
        ...
        Ok(#name_encoder_ident { buf, msg_offset: pos, pos: body_pos + #block_length_lit, _header: core::marker::PhantomData })
    };
    ...
    if total_tail > 0 {
        ...
        // Complete state: body/header byte views, encoded lengths on the final stage struct
        let complete_ident = &stage_idents[total_tail];
        ts.extend(quote::quote! {
            impl<'a> #complete_ident<'a> {
