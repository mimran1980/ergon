...
- [ ] In `codegen.rs`, add cap logic after computing `max_encoded_length`
- [ ] Cap at 65536 (64KB) with warning doc comment when exceeded
- [ ] Update doc to say "MAX_ENCODED_LENGTH exceeds the 64KB stack limit; use Vec for heap"
- [ ] `ENCODED_LENGTH` for fixed-size messages (already in place, no var-data)
- [ ] Regenerate golden file (`cargo test update_golden -- --ignored`)
- [ ] All tests pass

## Verification / Unit Testing
- [ ] Write a test `test_max_encoded_length_cap` in `sbe/tests/integration_tests.rs` that:
